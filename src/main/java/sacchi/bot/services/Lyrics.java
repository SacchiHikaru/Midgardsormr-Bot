...
public class Lyrics {
	public static void lyricsHandler(Message message, Music music) {
		if (message.getContent().equalsIgnoreCase("!Lyrics")) {
			message.reply(
					"Usage: !Lyrics [song name] to receive a link to the Song's lyrics. If the song has none, I will tell you so.");
		} else {
	...
	public static String lyricsCheck(Message message, Music music) {
		for (int i = 0; i < music.songList.size(); i++) {
			if (music.songList.get(i).getName().toLowerCase()
					.contains(message.getContent().toLowerCase().replace("!lyrics ", ""))) {
